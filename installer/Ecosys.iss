...
begin
  DownloadPage := CreateDownloadPage(SetupMessage(msgWizardPreparing), 'Die aktuelle sichere Ecosys-Version wird heruntergeladen.', @OnDownloadProgress);
end;
function NextButtonClick(CurPageID: Integer): Boolean;
var
  ArchivePath: String;
begin
  ...
  try
    ...
    MsgBox('Die sichere Ecosys-Version konnte nicht heruntergeladen oder entpackt werden.'#13#13'Bitte prüfe deine Internetverbindung und versuche es erneut.', mbCriticalError, MB_OK);
    Result := False;
  finally
    DownloadPage.Hide;
  end;

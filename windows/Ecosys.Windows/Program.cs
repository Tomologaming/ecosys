...
sealed class EcosysForm : Form
{
    ...
    sealed class DeviceGlyph:Control{public string Kind{get;set;}="pc";protected override void OnPaint(PaintEventArgs e){e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;using var p=new Pen(Text,2);if(Kind=="phone")e.Graphics.DrawRoundedRectangle(p,12,4,12,28,4,4);else if(Kind=="laptop"){e.Graphics.DrawRectangle(p,8,7,20,13);e.Graphics.DrawLine(p,5,25,31,25);}else{e.Graphics.DrawRectangle(p,7,6,22,15);e.Graphics.DrawLine(p,12,25,24,25);}}}
}
